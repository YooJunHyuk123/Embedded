...
class MainActivity : AppCompatActivity() {
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_CODE_PERMISSIONS) {
            if (grantResults.isNotEmpty() && grantResults.all { it == PackageManager.PERMISSION_GRANTED }) {
                ...
                Toast.makeText(this, "권한이 모두 승인되어야 앱을 사용할 수 있습니다.", Toast.LENGTH_SHORT).show()
                finish()
            }
        }
    ...
    private fun startPreview() {
        ...
        cameraDevice?.createCaptureSession(listOf(previewSurface), object : CameraCaptureSession.StateCallback() {
            ...
            override fun onConfigureFailed(session: CameraCaptureSession) {
                Toast.makeText(this@MainActivity, "미리보기 설정 실패", Toast.LENGTH_SHORT).show()
            }
        }, null)
    }
    ...
    private fun startRecording() {
        ...
        cameraDevice?.createCaptureSession(listOf(previewSurface, recordSurface),
            object : CameraCaptureSession.StateCallback() {
                ...
                override fun onConfigureFailed(session: CameraCaptureSession) {
                    Toast.makeText(this@MainActivity, "녹화 세션 실패", Toast.LENGTH_SHORT).show()
                }
            }, null)
    }
    ...
    private fun stopRecording() {
        ...
        try {
            mediaRecorder.stop()
            mediaRecorder.reset()
            Toast.makeText(this, "녹화 완료: ${videoFile.absolutePath}", Toast.LENGTH_LONG).show()

            uploadVideo(videoFile)

        } catch (e: Exception) {
            e.printStackTrace()
            Toast.makeText(this, "녹화 중 오류: ${e.message}", Toast.LENGTH_LONG).show()
        }
        ...
        Thread {
            try {
                ...
                val request = Request.Builder()
                    .url("http://172.31.52.23:8123/upload/")  // 실제 서버 주소로 변경 필요
                    .post(multipartBody)
                    .build()
                ...
                runOnUiThread {
                    if (response.isSuccessful) {
                        Toast.makeText(this, "서버 업로드 성공", Toast.LENGTH_SHORT).show()
                    } else {
                        Toast.makeText(this, "서버 업로드 실패: $body", Toast.LENGTH_LONG).show()
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
